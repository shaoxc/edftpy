...
class Test(unittest.TestCase):
    def test_optim(self):
        ...
        # posfile='sub_c_al.vasp'
        # posfile='al_111_si.vasp'
        # posfile='fcc.vasp'
        pplist = {'Al': path_pp+pp_al, 'C' :path_pp + pp_c, 'Si': path_pp + pp_si}
        denfiles= {'Al': data_path + 'al.gga.recpot.list', 'Si': data_path + 'si.gga.recpot.list'} 
        ftypes = {'Al': 'list', 'Si' :'list'}
        ...
        # index_a = np.arange(0, 72)
        index_a = np.arange(0, 2)
        # subsys_a, driver_a = self.gen_sub_ks(ions, grid, pplist, index_a, atomicd, xc_kwargs, ke_kwargs, emb_ke_kwargs = emb_ke_kwargs, gaussian_options = gaussian_options)
        subsys_a, driver_a = self.gen_sub_of(ions, grid, pplist, index_a, atomicd, xc_kwargs, ke_kwargs, emb_ke_kwargs = emb_ke_kwargs, gaussian_options = gaussian_options)
        ...
        # grho = io.read_density('ini.xsf')
        # ncharge = np.sum(subsys_a.density[:])
        # subsys_a.density[:] = gsystem.sub_value(grho, subsys_a.density)
        # subsys_a.density[:] += (ncharge-np.sum(subsys_a.density))/np.size(subsys_a.density)
        ...
        funcdicts = {'KE' :ke, 'XC' :xc, 'HARTREE' :hartree, 'PSEUDO' :pseudo}
        total_evaluator = Evaluator(**funcdicts)
        energy = total_evaluator(rho).energy
    ...
    def gen_sub_of(self, ions, grid, pplist = None, index = None, atomicd = None, xc_kwargs = {}, ke_kwargs = {}, emb_ke_kwargs = {}, gaussian_options= None, **kwargs):
        ...
        options = {"method" :'CG-HS', "maxiter": 820, "econv": 1.0e-6*ions.nat, "ncheck": 2, "opt_method" : 'full'}
        # options = {"method" :'CG-HS', "maxiter": 820, "econv": 1.0e-8*ions.nat, "ncheck": 2, "opt_method" : 'hamiltonian'}
        ke_kwargs = {'name' :'LMGPA', 'x' :0.0, 'y' :0.0, 'interp' :'hermite', 'kdd' :3, 'ratio':1.15, 'fd':3, 'kfmin' :1E-3, 'kfmax' :4.0, 'lumpfactor' : None, 'ldw' : 1.0/6.0}
        ...
        mixer = PulayMixer(predtype = 'kerker', predcoef = [1.0, kf, 1.0], maxm = 7, coef = [0.7], predecut = 0, delay = 1, restarted = False)
        # mixer = PulayMixer(predtype = 'kerker', predcoef = [1.0, kf, 1.0], maxm = 3, coef = [0.7], predecut = 0, delay = 1, restarted = True)
        # mixer = PulayMixer(predtype = 'resta', predcoef = [10, 0.68, 6.61], maxm = 7, coef = [0.2], predecut = None, delay = 0)
    ...
    def gen_sub_ks(self, ions, grid, pplist = None, index = None, atomicd = None, xc_kwargs = {}, ke_kwargs = {}, emb_ke_kwargs = {}, gaussian_options= None, **kwargs):
        ...
        # mixer = PulayMixer(predtype = 'kerker', predcoef = [1.0, 0.6, 1.0], maxm = 7, coef = [0.2], predecut = 0, delay = 1, restarted = False)
        mixer = PulayMixer(predtype = 'kerker', predcoef = [1.0, 0.6, 1.0], maxm = 7, coef = [0.7], predecut = 0, delay = 1, restarted = False)
